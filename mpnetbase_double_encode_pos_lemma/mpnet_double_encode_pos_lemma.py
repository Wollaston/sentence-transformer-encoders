...
modelB = SentenceTransformer("sentence-transformers/all-mpnet-base-v2")
...
class EnsembleEmbeddingsSentenceTransformer(SentenceTransformer):
    ...
    def encode(
        ...
        **kwargs,
        ...
        processed = " ".join(
            [str(token.text + "/" + token.lemma_ + "/" + token.pos_) for token in doc]
        )

        embeddingAProcessed = modelA.encode(processed, convert_to_tensor=True)
        embeddingBProcessed = modelB.encode(processed, convert_to_tensor=True)

        embedding = torch.cat(
            (embeddingAProcessed, embeddingBProcessed),
            dim=0,
        )
